...
## --------------------------------------------- ##
# Purpose:
## Process the precip data into a use-able (i.e., for analysis/visualization) format
...
(local_prec <- dir(path = file.path("data", "raw-climate"), pattern = "SXWC1"))

# Identify just the last file (if more than one exists, it's probably earlier/later downloads)
(raw_prec <- file.path("data", "raw-climate", local_prec[length(local_prec)]))
...
write.csv(x = prec_v99, row.names = F, na = '',
          file = file.path("data", "tidy", tidy_prec))
